...
class ActionModule(ActionBase):
    ...
    def _get_external_interface(self, net_name, required):
        ...
        condition = self._templar.template(condition)
        if condition:
            iface = self._templar.template("{{ '%s' | net_interface }}" % net_name)
            if iface:
                ...
                vlan = self._templar.template("{{ '%s' | net_vlan }}" % net_name)
    ...
    def _get_external_interface_facts(self, external_interfaces):
        ...
        bridge_suffix = self._templar.template("{{ network_bridge_suffix_ovs }}")
        ...
        return {
            ...
            "kolla_neutron_external_interfaces": ",".join(neutron_external_interfaces),
        }
